...
const Phonograph = {
  ...
  crossfade(result, source, target, length, context, factor) {
    ...
    if (factor > 0) this.frame = requestAnimationFrame(this.crossfade.bind(this, result, source, target, length, context, factor - 0.01));
  },
  updateArtwork(player) {
    const self = this;
    childProcess.exec(`osascript ${path.join(__dirname, '..', 'applescript', `update_art_${player.name.toLowerCase()}.scpt`)}`, (error, stdout, stderr) => {
      const tmp = path.join(__dirname, '..', '.tmp');
      if (!fs.existsSync(tmp)) fs.mkdirSync(tmp);
      let filePath = path.join(__dirname, '..', '.tmp', `${player.name.toLowerCase()}.png`),
          file = null;
      if (error) return;

      if (fs.existsSync(filePath)) {
        file = fs.readFileSync(filePath);
        let canvas = player.view.artwork,
        ...
        image.onload = function () {
          ...
          let border = 10, scaledSize = size / 2;
          ...
          target = context.getImageData(0, 0, size, size).data;
          ...
          bgImage.onload = function () {
            ...
            centerImage.onload = function () {
              ...
              // player.view.duration.style['border-color'] = `rgba(${fgColors[0].join(',')},0.5)`;
            };
          };
        };
      }
  ...
  parseTrackInfo(error, stdout, stderr) {
    let players = {},
        numberActive;
    if (!error) {
      players = {};
      ...
      for (const p in players) {
        ...
        // atom.workspaceView.trigger('phonograph:update', { player: this.players[p] });
      }
    }
  ...
  getColorDistance(a, b) {
    ...
    return Math.sqrt(Math.pow((dl / (1 * sl)), 2) + Math.pow((dc / (1 * sc)), 2) + Math.pow((dh / (1 * sh)), 2));
  },
};
